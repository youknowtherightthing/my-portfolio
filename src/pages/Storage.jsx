cat > src / pages / Storage.jsx << 'ENDOFFILE'
...
function Storage() {
    ...
    const handleUpload = (file) => {
        ...
        uploadTask.on('state_changed',
            ...
            async () => { setUploading(false); setProgress(0); await fetchFiles() }
        )
    }
    ...
    return (
        <div className="bg-black text-white min-h-screen px-6 py-16">
            <div className="max-w-4xl mx-auto">
                ...
                <p className="text-blue-400 text-xs font-mono tracking-widest mb-6">Uploaded Files ({files.length})</p>
                {files.length === 0 && <p className="text-gray-600 font-mono text-sm">No files uploaded yet.</p>}
                        ...
                        <div key={file.name} className="flex items-center justify-between border border-gray-800 rounded-2xl p-4 hover:border-blue-700 transition">
                            <div className="flex items-center gap-4">
                                ...
                                <div>
                                    ...
                                    <p className="text-gray-600 text-xs font-mono">Click Open or Copy Link to share</p>
                                </div>
                            </div>
                            <div className="flex items-center gap-3">
                                ...
                                <button onClick={() => handleDelete(file.ref, file.name)} className="border border-red-900 text-red-500 hover:bg-red-950 text-xs font-mono px-3 py-2 rounded-lg transition">Delete</button>
                            </div>
                        </div>
                    ))}
                </div>
